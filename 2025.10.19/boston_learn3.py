...
# Решила убрать из зависимых переменных категорийную переменную CHAS (по сути фиктивная перменная не имеющая реальных числовых данных). Также у нас ызыал сомнение перепенная AGE,
# но при тестовой выборке 10% исключение переменной AGE снизила показатели метрик.
...
# При уменьшении тестоой выборки до 10% и исключение категорийной переменной,  произошло существенное улучшение показателей технических метрик. 
# Однако, если сохранить этот же набор зависимых переменных и увеличить размер тестовой выборки до 20%, метрики существенно ухудшаются:
# RMSE = 4.2
# R2 = 28%
# R2 (adjusted) = 22%
