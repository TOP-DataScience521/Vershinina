...
# В данном примере я решила проверить как поведет себя модель, если добавить переменную TAX. Несмотря на то, что графический анализ нам не показал корреляции между этим показателем
# и целевой переменной, я усматриваю логику взаимосвязи стоимости жилья и налогов.
# отобранные зависимые переменные
effect_vars = ['CRIM', 'INDUS', 'NOX', 'RM', 'AGE', 'DIS', 'PTRATIO', 'LSTAT', 'TAX']
# целевая переменная
target_var = ['MEDV']
...
# Включение переменной TAX не улучшило показатели по сравнению с другими экспериментами, можно сделать вывод, что графический анализ и первоначальные расчеты об отсутвии 
# сильной корреляции между этой переменной и целевой переменной не существенны.
# Однако, при таком наборе зависимых переменных и  тестовой выборке 20%, итоговые метрики  ухудшаются незначительно:
# RMSE = 4.1
# R2 = 30%
# R2 (adjusted) = 23%

# Проведя несколько экспериментов с объемом тестовой выборки и включением/исключением зависимых переменных, наилучшие результаты метрик были в случае уменьшения тестовой выборки до 10%
# и исключении категорийной переменной.
